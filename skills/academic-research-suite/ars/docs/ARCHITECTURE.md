# ARS Codex Pipeline Architecture (v3.7.0)
...
`academic-pipeline/SKILL.md` and `pipeline_state_machine.md`); the diagrams
...
machine-verified first, then confirmed by the user — they are not skipped.
...
- **Flow diagram** (§2): macro view — which stage follows which, where loops exist, where gates block. Every rectangle ends in a post-stage user confirmation (elided for readability); decision markers call out the moments where the user chooses a branch.
- **Matrix** (§3): the only place where (stage × workflow × mode × data_level × artifacts × agent prompts × gate) all co-exist. Use this when asking "what happens at Stage X?" The Gate column lists both machine checks and the user-confirmation checkpoint that closes the stage.
...
- **Modes** (§9): reference when composing a pipeline invocation or a Codex
  `ars-*` alias request.
...
```mermaid
graph TD
    Pipeline[academic-pipeline<br/>orchestrator<br/>v3.6.5<br/>4 agent prompts]
...
timeline
    ...
    v3.6.7 : Downstream-agent PATTERN PROTECTION (Step 1+2)
           ...
           : check_v3_6_7_pattern_protection.py (58-test mutation suite in the Codex-vendored package)
    ...
    v3.7.0-codex : single Codex router skill
                 : ars-* alias emulation for upstream slash commands
                 : agents run as inline role prompts unless user requests delegation
                 : Codex vendored v3.6.7 block baseline for first-import SHA gate
